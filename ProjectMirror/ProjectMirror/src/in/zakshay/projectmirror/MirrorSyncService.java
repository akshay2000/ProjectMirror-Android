...
public class MirrorSyncService {
	...
	public void configureMobileService(String endPoint, String accessKey) {
		try {
			mobileServiceClient = new MobileServiceClient(endPoint, accessKey, context);
	...
	private <TEntity extends SyncableBase> Date getLastSyncedTime(Class<TEntity> clazz) {
	...
	private <TEntity extends SyncableBase> void setLastSyncedTime(Class<TEntity> clazz, Date recentUpdated) {
		DatabaseHelperBase dbHelperBase = getDbHelper();
		String entityName = clazz.getSimpleName();
		Dao<SyncTimeStamp, String> dao;
		try {
			...
			if(tableRow != null){
				...
				SyncTimeStamp newStamp = new SyncTimeStamp(entityName, recentUpdated);
				dao.create(newStamp);
			}
		} catch (SQLException e){
			e.printStackTrace();
	...
	public <TEntity extends SyncableBase> void synchronize(final Class<TEntity> clazz, final SyncCallback callback) throws SQLException {
		DatabaseHelperBase dbHelper = getDbHelper();
		final Dao<TEntity, Integer> localTable = dbHelper.getDao(clazz);
		
		MobileServiceTable<TEntity> remoteTable = (MobileServiceTable<TEntity>)mobileServiceClient.getTable(clazz);
		
		final Date lastSynced = this.<TEntity>getLastSyncedTime(clazz);
		final Date currentTimeStamp = lastSynced != null ? lastSynced: beginingOfTheTime;
		...
		.execute(new TableQueryCallback<TEntity>() {
			public void onCompleted(List<TEntity> result, 
				...
				if (exception == null) {
					...
					resolveConflicts(newLocalItems, newRemoteItems); //Modify the lists directly

					try {
						newTimeStamp[0] = applyRemoteToLocal(newRemoteItems, clazz, newTimeStamp[0]);
						if (newLocalItems.isEmpty()) {//The list is empty. No need to continue the method. Do maintenance now.
							setLastSyncedTime(clazz, newTimeStamp[0]);
							callback.onSyncComplete(null);
						}
						else {
							applyLocalToRemote(newLocalItems, clazz, newTimeStamp[0], callback);
						}
	...
	private <TEntity extends SyncableBase> void resolveConflicts(List<TEntity> newLocalItems, List<TEntity> newRemoteItems) {
		for(TEntity localItem : newLocalItems){
			...
			for(TEntity remoteItem: newRemoteItems){
				if(remoteItem.getRemoteId().equals(localId)){
					boolean removed = localItem.getLastModified().after(remoteItem.getLastModified()) ? newRemoteItems.remove(remoteItem) : newLocalItems.remove(localItem);
				}
			}
		}
	}
	
	private <TEntity extends SyncableBase> Date applyRemoteToLocal(List<TEntity> newRemoteItems, Class<TEntity> clazz, Date currentTimeStamp) throws SQLException {
		DatabaseHelperBase dbHelper = getDbHelper();
		final Dao<TEntity, Integer> localTable = dbHelper.getDao(clazz);
		...
		for(TEntity remoteItem : newRemoteItems){
			...
			newTimeStamp = newTimeStamp.after(remoteItem.getLastSynchronized()) ? newTimeStamp : remoteItem.getLastSynchronized();
		}
		return newTimeStamp;
	}
	
	private <TEntity extends SyncableBase> void applyLocalToRemote(List<TEntity> newLocalItems, final Class<TEntity> clazz, Date currentTimeStamp, final SyncCallback callback) throws SQLException {
		DatabaseHelperBase dbHelper = getDbHelper();
		final Dao<TEntity, Integer> localTable = dbHelper.getDao(clazz);
		...
		for(TEntity localItem: newLocalItems) {
			if(localItem.getRemoteId() == null){
				remoteTable.insert(localItem, new TableOperationCallback<TEntity>() {
					public void onCompleted(TEntity entity, Exception exception, ServiceFilterResponse response){
						countdown[0]--;
						if(exception == null) {
							try {
								localTable.update(entity);
								newTimeStamp[0] = newTimeStamp[0].after(entity.getLastSynchronized()) ? newTimeStamp[0] : entity.getLastSynchronized();
							} catch (SQLException e) {
								e.printStackTrace();
							}
						}
				...
				remoteTable.update(localItem,  new TableOperationCallback<TEntity>() {
					public void onCompleted(TEntity entity, Exception exception, ServiceFilterResponse response){
						countdown[0]--;
						if(exception == null) {
							try {
	...
	public <TEntity extends SyncableBase> void addRemoteItem(TEntity itemToAdd) throws InterruptedException {
...
//					Log.e("Bingosss", i.getRemoteId());
	...
	private <TEntity extends SyncableBase> TEntity getItemByRemoteId(TEntity remoteItem){
		DatabaseHelperBase dbHelper = getDbHelper();
		Dao<TEntity, Integer> dao;
		try {
